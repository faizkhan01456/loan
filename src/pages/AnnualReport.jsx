...
import { BarChart2, ChevronDown, Calendar, FileText } from "lucide-react";
...
export default function AnnualReports() {
  ...
  const availableYears = Object.keys(annualData);
  ...
  const currentReports = annualData[selectedYear] || [];
    ...
    <div className={`py-16 px-4 ${LIGHT_BG} font-sans min-h-screen`}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center md:text-left mb-12">
          <BarChart2 className={`w-10 h-10 md:mx-0 mx-auto mb-3 ${PRIMARY_COLOR}`} />
          <h1 className="text-4xl md:text-5xl font-extrabold text-gray-900 mb-2">
            Annual Reports and <span className={PRIMARY_COLOR}>Returns</span>
          </h1>
          <p className="text-lg text-gray-600 max-w-3xl md:mx-0 mx-auto">
            Review our statutory annual reports and returns filed with the regulators.
          </p>
        </div>
          ...
          <div className="flex-grow w-full">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {currentReports.map((report, index) => (
                <div 
                  key={index} 
                  className={`relative bg-white p-6 rounded-xl shadow-lg border-t-4 ${report.isLatest ? 'border-blue-600 shadow-xl' : 'border-gray-100'} 
                  transition duration-300 hover:shadow-2xl hover:-translate-y-1`}
                >
                  <FileText className={`w-8 h-8 mb-3 ${PRIMARY_COLOR}`} />
              ...
              {currentReports.length === 0 && (
                <p className="text-gray-500 col-span-full">No annual documents available for the selected year.</p>
              )}
            </div>
          </div>
        </div>
